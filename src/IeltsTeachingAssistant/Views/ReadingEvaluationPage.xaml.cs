...
public sealed partial class ReadingEvaluationPage : Page
{
    ...
    {
        string passage = PassageInput.Text;
        string answers = AnswersInput.Text;

        if (string.IsNullOrWhiteSpace(passage) || string.IsNullOrWhiteSpace(answers))
        {
            ShowInfoBar(InfoBarSeverity.Warning, "Please provide both the reading passage and the student's answers.");
            return;
        }

        AnalyzeButton.IsEnabled = false;
        GradingProgress.IsActive = true;
        ShowInfoBar(InfoBarSeverity.Informational, "Analyzing passage and answers with Gemini AI...");

        try
        {
            var analysis = await _vertexAIService.AnalyzeReadingAsync(passage, answers);
    ...
    private async void SaveButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (string.IsNullOrWhiteSpace(passage) || string.IsNullOrWhiteSpace(answers) || string.IsNullOrWhiteSpace(analysis) || analysis.StartsWith("AI diagnostic"))
        {
        ...
        try
        {
            ...
            ShowInfoBar(InfoBarSeverity.Success, $"Successfully saved Reading Evaluation for {selectedStudent.Name} with Band {bandScore:F1}!");
        }
        catch (Exception ex)
        {
