...
namespace IeltsTeachingAssistant.Views
{
    public sealed partial class ListeningEvaluationPage : Page
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(ScriptInput.Text) || string.IsNullOrWhiteSpace(AnswersInput.Text))
            {
                ShowStatus("Please enter both the listening script and student answers.", InfoBarSeverity.Warning);
                return;
            }

            GradingProgress.IsActive = true;
            AnalyzeButton.IsEnabled = false;
            ShowStatus("Analyzing listening script and spelling with AI...", InfoBarSeverity.Informational);

            try
            {
                var analysis = await _vertexAiService.AnalyzeListeningAsync(ScriptInput.Text, AnswersInput.Text);
        ...
        private async void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                var eval = new ListeningEvaluation
                {
                    ...
                    ListeningScript = ScriptInput.Text,
